...
app.post("/wms_inventory/:integrationName/:actionType/:companyId/:accountDatasourceId/:datasourceId/:action", async (c) => {
    ...
    const body = rawBody ? JSON.parse(rawBody) : {};
     ...
     const urlParamComponents: UrlParamComponents = { companyId, datasourceId, accountDatasourceId, integrationName, actionType, action };
     const skipCredentials = actionType === "bulk" && action === "sync";
     const credentials = skipCredentials
       ? {}
       : await getCredentials(companyId, accountDatasourceId, integrationName);
     if (actionType === "bulk" && action === "sync") {
       EdgeRuntime.waitUntil(
         integrationAction(body, credentials, urlParamComponents).catch((error) => {
           console.error("Background inventory bulk sync failed:", error);
         }),
       );
     ...
     if (actionType === "webhook" && action !== "subscribe" && action !== "unsubscribe") {
       EdgeRuntime.waitUntil(
         integrationAction(body, credentials, urlParamComponents).catch((error) => {
